...
def update_snake_position(snake_position, theta, amplitude, wavelength, t):
    for i in range(1, n_segments):
        ...
        y_new += amplitude * np.sin(t * wavelength * (2 * np.pi) / 2) # amplitude * np.sin(wavelength * np.deg2rad(i*360/n_joints) * t + phase)
        snake_position[i] = np.array([x_new, y_new])
    return snake_position

# Define the function to animate the snake
def animate(i):
    global snake_position, theta, amplitude, wavelength
    ...
    theta = [offset + amplitude * np.sin(wavelength * ((2 * np.pi) / 2) * i * dt) for i in range(n_joints)] # offset + amplitude * np.sin(wavelength * np.deg2rad(i*360/n_joints) * i * dt) for i in range(n_joints)
    
    # Update the snake position
    snake_position = update_snake_position(snake_position, theta, amplitude, wavelength, i*dt)
